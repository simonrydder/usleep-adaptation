                ...
                try:
                    ...
                    for rec_key in rec_keys:
                        ...
                        if self.channels_to_pick != None:
                            psg_keys = [
                                k for k in psg_keys if k in self.channels_to_pick
                            ]
                    ...
                    print(
                        f"Did not find subject {subj_key} in dataset EESM with split type {self.split_type}"
                    )
                    continue
...
class Full_Train_Dataset_Sampler(IPipe):
    ...
    def process(self, index):
        ...
        rest_epochs = num_epochs - (
            math.floor(num_epochs / self.window_length) * self.window_length
        )
                ...
                try:
                    ...
                    for rec_key in rec_keys:
                        ...
                        for c in psg_keys:
                            ...
                            whole_windows = math.floor(
                                len(channel_data) / 128 / 30 / self.window_length
                            )
                    ...
                    print(
                        f"Did not find subject {subj_key} in dataset EESM with split type TRAIN"
                    )
                    continue
